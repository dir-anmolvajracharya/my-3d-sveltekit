<script lang="ts">
    ...
    const getRandomMaterialParam = () => new THREE.MeshStandardMaterial(gsap.utils.random(materialParams))
...
<Threlte.Group position={position.map(p=>p*2)}>
    <Float speed={5*rate} rotationSpeed={5*rate} rotationIntensity={6*rate} floatIntensity={5*rate}>
        <Threlte.Mesh {geometry} material={getRandomMaterialParam()}></Threlte.Mesh>
    </Float>
</Threlte.Group>