	...
	onMount(() => {
		...
		tl.fromTo(
			'.tech-row',
			{
				x: (index) => {
					if (index % 2 === 0) {
						return gsap.utils.random(400, 600);
					} else {
						return gsap.utils.random(-600, -400);
					}
				}
			},
			{
		...
		>
			{#each Array(15) as _, index}
				<span
					...
					style="color: {index === 7 && tech_color ? tech_color : 'inherit'}">{tech_name}</span
				>
				<span class="text-3xl"><IconCircle /></span>
			{/each}
		</div>
	{/each}
</section>
